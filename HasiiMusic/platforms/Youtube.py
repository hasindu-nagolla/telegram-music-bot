...
from HasiiMusic.utils.downloader import yt_dlp_download
from HasiiMusic.utils.errors import capture_internal_err
from HasiiMusic.utils.formatters import time_to_seconds
from HasiiMusic.utils.tuning import YTDLP_TIMEOUT, YOUTUBE_META_MAX, YOUTUBE_META_TTL
...
class YouTubeAPI:
    ...
    async def url(self, message: Message) -> Optional[str]:
        msgs = [message] + ([message.reply_to_message]
    ...
    async def details(
        ...
        try:
            ...
            if not info:
                raise ValueError(
                    "No results from youtubesearchpython (VideosSearch)")
        except Exception as search_err:
            raise ValueError("Video not found", {
                             "cause": str(search_err)}) from search_err
        ...
        try:
            ...
            stdout, stderr = await _exec_proc(
                "yt-dlp", *(_cookies_args()
                            ), "--dump-json", "--no-warnings", prepared_link
            )
