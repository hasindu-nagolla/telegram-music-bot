...
from HasiiMusic.core.dir import CACHE_DIR, DOWNLOAD_DIR
            ...
            ytdlp_task = asyncio.create_task(
                run_with_semaphore(
                    loop.run_in_executor(
                        None, download_with_ytdlp_sync, link, "bestaudio/best")
                )
            )
            api_task = asyncio.create_task(
                api_download_audio(link)) if USE_AUDIO_API else None
            if api_task:
    ...
    if type == "video":
        ...
        async def run():
            ytdlp_task = asyncio.create_task(
                run_with_semaphore(
                    loop.run_in_executor(
                        None, download_with_ytdlp_sync, link, "best[height<=?720]/best")
                )
            )
            api_task = asyncio.create_task(
                api_download_video(link)) if USE_VIDEO_API else None
            if api_task:
                return await race_ytdlp_and_api(ytdlp_task, api_task, title or "Unknown")
            result = await ytdlp_task
            if result and title:
